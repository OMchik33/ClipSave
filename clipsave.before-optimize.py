...
def build_youtube_download_attempts(mode: str, format_id: str | None, max_height: int) -> list[tuple[str, str]]:
    ...
    return dedupe_preserve_order(
        [
            ("основной (bv*+ba)", f"bv*{vf}+ba/b{vf}/b"),
            ("H.264 MP4 (самый стабильный)", f"bv*[vcodec^=avc1]{vf}+ba[ext=m4a]/b[ext=mp4]{vf}/b"),
            ("готовый MP4 поток", f"b[ext=mp4]{vf}/b{vf}/b"),
            ("любой готовый", f"b{vf}/b"),
        ]
    )
...
def build_youtube_audio_first_attempts(max_height: int) -> tuple[list[tuple[str, str]], list[tuple[str, str]]]:
    ...
    audio_variants: list[tuple[str, str]] = [
        ...
        ("M4A 128 kbps", "140"),
        ("Opus 50 kbps", "249"),
        ("Opus 130 kbps DRC", "251-drc"),
        ("Opus 70 kbps DRC", "250-drc"),
        ("M4A DRC", "140-drc"),
        ("любое Opus", "ba[acodec^=opus]"),
        ("любое M4A", "ba[ext=m4a]"),
        ("любое аудио", "ba/b"),
    ]
