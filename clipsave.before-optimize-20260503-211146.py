...
def build_base_ydl_opts(user_id: str, *, skip_download: bool, quiet: bool, task_id: str | None = None) -> dict[str, Any]:
    ...
    opts: dict[str, Any] = {
        ...
        # === КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ ДЛЯ YOUTUBE ===
        "extractor_args": {
            "youtube": {
                "player_skip": ["webpage"],
    ...
    if cookie_file:
        ...
        opts["extractor_args"]["youtube"]["player_client"] = ["ios", "web"]  # с cookies лучше ios+web
...
def build_youtube_download_attempts(mode: str, format_id: str | None, max_height: int) -> list[tuple[str, str]]:
    ...
    if mode == "audio":
        return dedupe_preserve_order([
            ...
            ("M4A/Opus", "ba[ext=m4a]/ba[acodec^=opus]/ba/b"),
        ])

    if mode == "pick" and format_id:
        return dedupe_preserve_order([
            (f"Формат {format_id}", f"{format_id}+ba/{format_id}"),
        ])

    return dedupe_preserve_order([
        ("Лучшее качество (ios/android)", "bestvideo+bestaudio/best"),
        ("H.264 + аудио", f"bv*[vcodec^=avc1]{vf}+ba[ext=m4a]/b[ext=mp4]{vf}"),
        ("Любой MP4", f"b[ext=mp4]{vf}/bv*+ba/b"),
        ("Любой формат", "bv*+ba/b"),
    ])
...
def sync_analyze_url(user_id: str, url: str) -> dict[str, Any]:
    ...
    allow_unlimited = bypass_user_limits or setting_bool(settings, "allow_unlimited_quality") or max_height == 0

    opts_info = build_base_ydl_opts(user_id, skip_download=True, quiet=True)
    # Убираем ограничение высоты при анализе, чтобы видеть все форматы
    opts_info["format"] = "bv*+ba/b/bestvideo+bestaudio/best"
    ...
    for f in formats:
        ...
        if not fid or ext == "mhtml" or "storyboard" in str(fid).lower():
            continue
